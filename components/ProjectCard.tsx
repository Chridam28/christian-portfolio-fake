...
export function ProjectCard({ project, eager = false }: { project: Project; eager?: boolean }) {
  ...
  return (
    <article className="portfolio-card reveal" data-project-kind={project.kind} data-project-categories={project.categories.join(' ')}>
      <Link href={`/progetti/${project.slug}`} className="portfolio-card-visual" data-analytics={project.kind === 'real' ? 'project_open' : 'demo_open'} data-project={project.slug}>
      ...
      <div className="portfolio-card-body">
        ...
        <h2><Link href={`/progetti/${project.slug}`}>{project.title}</Link></h2>
        <p>{project.shortDescription}</p>
        <div className="card-footer">
          <span>{project.sector}</span>
          <Link href={`/progetti/${project.slug}`}>Dettagli progetto <span aria-hidden="true">↗</span></Link>
        </div>
        {project.externalUrl && (
          <a
