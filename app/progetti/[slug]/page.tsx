...
export default async function ProjectPage({ params }: ProjectPageProps) {
  ...
  return <main>
    <section className={`project-hero theme-${project.theme}`}>
      ...
      <div className="shell project-title-grid"><h1>{project.title}</h1><div><p>{project.shortDescription}</p>{project.externalUrl ? <a className="button button-accent" href={project.externalUrl} target="_blank" rel="noreferrer" data-analytics="external_site_click" data-project={project.slug}>Visita il sito <span aria-hidden="true">↗</span></a> : <span className="unavailable-link">Demo non ancora disponibile online</span>}</div></div>
      <div className="shell project-hero-visual"><ProjectPreview project={project} eager /></div>
    </section>
    <section className="project-facts shell reveal"><dl><div><dt>{isReal ? 'Cliente' : 'Scenario'}</dt><dd>{isReal ? project.title : project.scenarioLabel}</dd></div><div><dt>Settore</dt><dd>{project.sector}</dd></div><div><dt>Anno</dt><dd>{project.year}</dd></div><div><dt>Intervento</dt><dd>{project.typeLabel}</dd></div></dl></section>
    <section className="case-intro shell"><p className="section-index">{isReal ? 'La necessità' : 'Esigenze simulate'}</p><div className="reveal"><h2>{project.problem}</h2><p>{project.fullDescription}</p></div></section>
    <section className="case-solution"><div className="shell case-solution-grid"><div className="reveal"><p className="eyebrow">La soluzione</p><h2>Struttura, identità e uso quotidiano.</h2><p>{project.solution}</p></div><div className="feature-list reveal"><p className="list-title">{isReal ? 'Principali funzioni' : 'Funzioni dimostrate'}</p>{project.features.map((feature, index) => <div key={feature}><span>{String(index + 1).padStart(2, '0')}</span><p>{feature}</p></div>)}</div></div></section>
    <section className="objectives shell"><div><p className="eyebrow dark">Obiettivi</p><h2>{isReal ? 'Le priorità che hanno guidato il progetto.' : 'Le priorità dello scenario ipotetico.'}</h2></div><ol>{project.objectives.map((objective, index) => <li className="reveal" key={objective}><span>{String(index + 1).padStart(2, '0')}</span>{objective}</li>)}</ol></section>
    ...
    <section className="project-services shell"><div><p className="eyebrow dark">{isReal ? 'Servizi forniti' : 'Ambiti esplorati'}</p><h2>Cosa comprende la scheda.</h2></div><div className="tag-list">{project.services.map((service) => <span key={service}>{service}</span>)}{project.technologies.map((technology) => <span key={technology}>{technology}</span>)}</div></section>
    {!isReal && project.possibleCustomizations && <section className="customizations shell"><p className="section-index">Possibili personalizzazioni</p><div>{project.possibleCustomizations.map((item) => <span key={item}>{item}</span>)}</div></section>}
    <ContactBand />
  </main>;
}
