...
const faqs = [
  ['Quanto tempo serve?', 'Dipende da contenuti e complessità. Dopo il confronto iniziale preparo una stima realistica con fasi e scadenze; un sito essenziale richiede normalmente alcune settimane.'],
  ['Quanto costa un sito?', 'Non uso un listino standard perché struttura, contenuti e funzioni cambiano molto. Dopo aver capito il progetto preparo un preventivo chiaro, con cosa è incluso e modalità di pagamento.'],
  ['Dominio e hosting sono inclusi?', 'Posso occuparmi della configurazione e indicarti la soluzione più adatta. Costi e intestazione dei servizi vengono chiariti nel preventivo.'],
  ['Quali materiali devo fornire?', 'Partiamo da informazioni su attività, servizi, pubblico e obiettivi. Se hai già testi, fotografie e identità visiva li valuto; altrimenti definiamo insieme cosa serve.'],
  ['Puoi aiutarmi con testi e fotografie?', 'Posso organizzare e rifinire i contenuti per il web. Per servizi specialistici di copywriting o fotografia, quando necessari, concordiamo prima come procedere.'],
  ['Quante revisioni sono previste?', 'Il numero e i momenti di revisione vengono indicati nel preventivo. Preferisco revisioni raccolte e ragionate, così il progetto procede senza confusione.'],
  ['Ti occupi anche della manutenzione?', 'Sì. Possiamo definire assistenza continuativa oppure interventi programmati, in base a quanto spesso il sito va aggiornato.'],
  ['Il sito rimane mio?', 'Sì. Proprietà, accessi e materiali vengono gestiti in modo trasparente e specificati negli accordi di progetto.'],
  ['Come funzionano i pagamenti?', 'Le modalità vengono definite nel preventivo e normalmente seguono le principali fasi del lavoro, evitando un unico pagamento a fine progetto.'],
  ['Possiamo lavorare a distanza?', 'Certo. Lavoro da Trieste con clienti in tutta Italia attraverso call, materiali condivisi e revisioni online.'],
];
  ...
  return (
    <main>
      <section className="hero" id="home">
        <div className="hero-grid shell">
          ...
          <div className="hero-canvas" aria-hidden="true">
            ...
            <div className="canvas-card canvas-card-primary"><span className="mini-label">PROGETTO / 01</span><strong>Identità</strong><div className="mini-lines"><i /><i /><i /></div></div>
            <div className="canvas-card canvas-card-secondary"><span>Mobile first</span><b>375</b></div>
      ...
      <section className="method-section shell" id="metodo">
        ...
        <div className="method-grid">{method.map(([number, title, copy]) => <article className="method-card reveal" key={number}><span>{number}</span><div className="method-marker" aria-hidden="true" /><h3>{title}</h3><p>{copy}</p></article>)}</div>
      </section>

      <section className="reasons-section"><div className="shell reasons-grid">
        <div className="reveal"><p className="eyebrow dark">05 / Lavorare insieme</p><h2>Più diretto.<br /><em>Più personale.</em></h2><p>Collaborare con un freelance significa parlare sempre con la persona che prende le decisioni e mette mano al progetto.</p></div>
        <ol>{reasons.map((reason, index) => <li className="reveal" key={reason}><span>{String(index + 1).padStart(2, '0')}</span><p>{reason}</p></li>)}</ol>
      </div></section>

      <section className="about-preview shell"><div className="about-grid reveal"><p className="section-index">06 / Chi sono</p><div><h2>Un professionista indipendente, non un’agenzia travestita.</h2><p>Sono Christian e lavoro a Trieste. Seguo ogni progetto in prima persona, dalla struttura alle ultime verifiche, per mantenere coerenza, tempi chiari e un rapporto semplice con chi mi affida il proprio spazio digitale.</p><a className="button button-dark" href="/chi-sono">Qualcosa su di me <span aria-hidden="true">↗</span></a></div><div className="about-signature" aria-hidden="true"><span>CD</span><i /></div></div></section>

      <section className="faq-section shell">
        <div className="faq-heading reveal"><p className="eyebrow dark">07 / Domande frequenti</p><h2>Prima di iniziare.</h2><p>Le risposte essenziali per capire come possiamo lavorare insieme.</p></div>
        <div className="faq-list">{faqs.map(([question, answer], index) => <details className="reveal" key={question}><summary><span>{String(index + 1).padStart(2, '0')}</span>{question}<b aria-hidden="true">+</b></summary><p>{answer}</p></details>)}</div>
      </section>
      <ContactBand />
    </main>
  );
